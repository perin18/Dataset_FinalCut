...
import time
import sys
from Catia_utils import macro2pc
from Geometry_utils import read_ply
import win32com.client

sys.path.append("..")
# 解码前数据对应的点云
PC_ROOT = "C:/Users/45088/Desktop/deepcad_ours/data/DeepCAD_data_32_PC"
data_path = "C:/Users/45088/Desktop/deepcad_ours/data/DeepCAD_data_32"
sample_points = 2000
...
def process_one(path):
    ...
    data_id = path[-17:-7]
    # 对应解码前的点云数据
    gt_pc_path = os.path.join(PC_ROOT, data_id + '.ply')
    # 如果没有，自己做一个，然后存储，若失败则return None
    if not os.path.exists(gt_pc_path):
        gt_vec_path = os.path.join(data_path, data_id[:4], data_id + '.h5')
        with h5py.File(gt_vec_path, 'r') as fp:
            gt_vec = fp["vec"][:].astype(np.float)
        try:
            gt_pc = macro2pc(catia, gt_vec, sample_points, True, os.path.join(PC_ROOT, data_id))
        except Exception as e:
            print("convert gt pc failed:", data_id)
            return None
    else:
        gt_pc = read_ply(gt_pc_path)

    out_pc = macro2pc(catia, out_vec, sample_points)
    if gt_pc is None or out_pc is None:
        return None
...
def run(args):
    ...
    dists = []
    # 若已存在，并非删除而是继续，将信息读入dists
    if os.path.exists(save_path):
        with open(save_path, 'r') as fp:
            print()
            record_res = fp.readlines()
            n_processed = len(record_res)
...
# 解码后数据
parser.add_argument('--src', type=str, default="C:/Users/45088/Desktop/deepcad_ours/proj_log/DeepCAD/results/test_1000")
