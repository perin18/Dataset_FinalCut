...
def create_Body_CATIA(part, extrude_op, cur_body_ref=None, is_first=False, sketch_offset=0, hole_sketch_map=None, only_sketch=False):

    profile = deepcopy(extrude_op.sketch_profile)  # use copy to prevent changing extrude_op internally
    if only_sketch:
        profile.denormalize(100, size=ARGS_N)
    else:
        profile.denormalize(extrude_op.sketch_size, size=ARGS_N)
    ...
    # 两点，前者为原点，后者用于构成法线
    if only_sketch:
        hybridShapePointCoord1 = hybridShapeFactory.AddNewPointCoord(0, 0, 0)
        hybridShapePointCoord2 = hybridShapeFactory.AddNewPointCoord(0, 0, 1)
    else:
        hybridShapePointCoord1 = hybridShapeFactory.AddNewPointCoord(sketch_plane.origin[0], sketch_plane.origin[1], sketch_plane.origin[2])
        hybridShapePointCoord2 = hybridShapeFactory.AddNewPointCoord(sketch_plane.origin[0] + sketch_plane.normal[0], sketch_plane.origin[1] + sketch_plane.normal[1], sketch_plane.origin[2] + sketch_plane.normal[2])
    reference1 = part.CreateReferenceFromObject(hybridShapePointCoord1)
    ...
    if only_sketch:
        sketch.SetAbsoluteAxisData(tuple([0, 0, 0,
                                          1, 0, 0,
                                          0, 1, 0]))
    else:
        sketch.SetAbsoluteAxisData(tuple([sketch_plane.origin[0], sketch_plane.origin[1], sketch_plane.origin[2],
...
def create_Feature_on_select(doc, part, op, cur_body_ref, catia, sketch_offset, body, hole_sketch_map):
    ...
    if isinstance(op, Hole):
        ...
        limit2.LimitMode = EXTENT_TYPE.index(op.bottom_mode)
        if op.bottom_mode == "UpToPlaneLimit" or op.bottom_mode == "UpToSurfaceLimit":
            hole_reference = parse_BrepName(op.select_list[0], True, sketch_offset=sketch_offset, hole_sketch_map=hole_sketch_map)
            limit2.LimitingElement = hole_reference
...
def create_CAD_CATIA(cad_seq: Macro_Seq, catia, doc, part, remove_bug=False, only_sketch=False):
    if not remove_bug:
        if only_sketch:
            create_Body_CATIA(part, cad_seq.extrude_operation[0], is_first=True, only_sketch=only_sketch)
            for i in cad_seq.extrude_operation[1:]:
                create_Body_CATIA(part, i, is_first=False, only_sketch=only_sketch)
            # 放大以便观察
...
def macro2pc(catia, vec, n_points, is_save=False, save_path=None):
    catia.visible = 1
    ...
    tmp_path = os.path.join(sys.path[0], "test.stl")
    if os.path.exists(tmp_path):
        os.remove(tmp_path)
    partDocument1.ExportData(tmp_path, "stl")
    doc.close()
    out_mesh = trimesh.load(tmp_path)
    ...
    if type == 'Face':
        ...
        # 若为Shell，则视为Multiply_Face
            ...
            if body_name == 'Chamfer' or body_name == 'EdgeFillet':
                ...
                topo_no = int(sketch_name[sketch_name.find(';') + 1:] == '5')
                select = Select('Face', body_name, body_no, topo_no, [], [], {})
            else:
                body_no = int(name_str[name_str.find('.') + 1:name_str.find(';')])
...
# 读取当前路径中的宏文件、bounding box并解析
def process_on(input_path, catia, doc, remove_bug=False, just_test=False, macro_test=False):
    if macro_test:
        input_macro = sorted(glob.glob(os.path.join(input_path, "*.catvbs")))[-1]
    else:
